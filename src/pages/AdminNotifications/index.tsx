...
// Đã cập nhật: bỏ MOCK_NOTIFICATIONS, type filter (BE log không có type)
// activity expandable row vẫn còn nhưng sẽ rỗng (BE không trả timeline)
import React, { useEffect, useState } from 'react';
import {
  Row, Col, Card, Table, Select, Input, Button, Space, Tag,
  message, Modal, Typography, Statistic, Progress, Avatar, Tooltip,
} from 'antd';
import {
...
import type { NotificationItem, NotificationStatus } from '@/services/notifications';
...
const AdminNotificationsPage: React.FC = () => {
  ...
  const loadFiltered = async () => {
    setLoading(true);
    try {
      const data = await listNotifications({ status: statusFilter, q: q || undefined });
      setItems(data);
    } catch (err: any) {
  ...
  useEffect(() => { loadAll(); }, []);
  useEffect(() => { loadFiltered(); }, [statusFilter, q]);
  ...
  const columns = [
    {
      ...
      render: (_: any, rec: NotificationItem) => (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <Avatar style={{ backgroundColor: '#2f54eb', marginRight: 8 }}>
            {rec.recipientEmail.charAt(0).toUpperCase()}
          </Avatar>
          <div>
            <div style={{ fontWeight: 700 }}>{rec.recipientEmail}</div>
          </div>
        </div>
      ),
    },
    {
      ...
      dataIndex: 'subject',
      key: 'subject',
      render: (s: string) => <Text ellipsis style={{ maxWidth: 300 }}>{s}</Text>,
    },
    {
      title: 'Thời gian',
      dataIndex: 'sentAt',
      key: 'sentAt',
      render: (v: string) => <div style={{ color: '#888' }}>{new Date(v).toLocaleString()}</div>,
    },
    {
      title: 'Lần gửi lại',
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: (s: NotificationStatus) => <StatusTag status={s} />,
    },
    {
      title: 'Hành động',
      key: 'actions',
      ...
      render: (_: any, rec: NotificationItem) => (
        <Space>
          <Tooltip title="Làm mới danh sách">
            <Button size="small" onClick={loadFiltered} icon={<ReloadOutlined />} />
          </Tooltip>
          {rec.status === 'FAILED' && (
            <Button
              type="primary"
  ...
  return (
    <div className="admin-notifications-page">
      <Row justify="space-between" align="middle" style={{ marginBottom: 12 }}>
        ...
        <Col>
          <Space>
            <Button icon={<ReloadOutlined />} onClick={() => { loadFiltered(); loadAll(); }}>
              Làm mới
            </Button>
          </Space>
        </Col>
        ...
        <Row gutter={12} align="middle">
          <Col xs={24} sm={8} md={6}>
            <Select
              ...
              style={{ width: '100%' }}
              value={statusFilter}
              onChange={(v) => setStatusFilter(v as NotificationStatus)}
            >
